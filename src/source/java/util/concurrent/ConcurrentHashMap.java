...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
     ...
     */
    static final int DEFAULT_INITIAL_CAPACITY = 16;//初始化容量// 默认初始值，必须是2的幕数
     ...
     * otherwise specified in a constructor.
     */
    static final int DEFAULT_CONCURRENCY_LEVEL = 16;//并发级别 指的是segment个数
     ...
     * using ints.
     */
    static final int MAXIMUM_CAPACITY = 1 << 30;// 用于计算Segment数组中的每一个segment的HashEntry[]的最大容量（2的30次方）
     ...
     */
    final Segment<K,V>[] segments;//将整个hashmap分成几个小的map,每个segment都是一个锁;与hashtable相比,这么设计对于put,remove等操作
                                    //可以减少并发冲突,对不属于同一个片段的节点可以并发操作,提高了性能
     ...
     *///本质segment类就是一个小的hashmap,里面table数组存储了各个节点的数据,继承了reentrantLock,可以作为互斥锁使用
    static final class Segment<K,V> extends ReentrantLock implements Serializable {
         ...
         */
        transient int count;// 该Segment中的包含的所有HashEntry中的key-value的个数
         ...
         * loadFactor)</tt>.)
         */
        transient int threshold;//元素个数超出了这个值就扩容 threshold==(int)(capacity * loadFactor)
//值得注意的是，只是当前的Segment扩容，所以这是Segment自己的一个变量，而不是ConcurrentHashMap的
     ...
     */
    @SuppressWarnings("unchecked")
    public ConcurrentHashMap(int initialCapacity,//这里以默认的无参构造器参数为例，initialCapacity==16，loadFactor==0.75f，concurrencyLevel==16
